 ...
 * VTK 核心引擎入口
 *
 * 职责分区（按区域注释划分，为未来提取 Manager 类做准备）：
 *   ┌─ [SCENE]       场景生命周期：initScene / destroyScene / triggerInitScene / handleReset
 *   ├─ [SELECTION]   选择交互：handleSelection / handlePolygonSelection / handleDelete / handleSpace / handleEscape
 *   ├─ [CAMERA]      相机控制：getCamera / setCamera / switchView / getFrontViewCameraInfo
 *   ├─ [FILE]        文件 I/O：getFileBuffer / getNowFile / getSingleNowFile / triggerFile / triggerFinalFile
 *   └─ [VISUAL]      可视化渲染：addLine / addSpline / addToothWidthLabels / renderToothOrder / toggleLight
 */
...
import {
  setRenderer,
  setSelectorDataSource,
  getSelectorDataSource,
  setApiSpecificRenderWindow,
  getHlCells,
} from "./context"
...
let toothWidths: Record<string, Record<string, Record<string, AnyObj>>> = {
  up: { sharp: {}, frontMolar: {}, molar: {} },
  down: { sharp: {}, frontMolar: {}, molar: {} },
}
...
setSelectorDataSource(new SelectorDataSource())
...
export default (function () {
  return function init(container: HTMLElement) {
    ...
    setApiSpecificRenderWindow(apiSpecificRenderWindow)
    ...
    function _triggerInitScene(
      ...
      for (let i = 0; i < models.length; i++) {
        if (type === "vtp") {
          const buffer = isArrayBuffer(buffers[i])
            ? buffers[i]
            : textEncoder?.encode(buffers[i] as unknown as string)
    ...
    const height = DeviceUtils.getContainerHeightPx()
    boxSelector.onBoxSelectChange(({ selection }: { selection: [number, number, number, number] }) => {
      const [xmin, xmax, ymin, ymax] = selection
      logger.debug("Apply selection:", [xmin, height - ymax, xmax, height - ymin])
      const selectorDS = (getSelectorDataSource() ?? new SelectorDataSource()) as SelectorDataSource
      selectorDS.clear()
      selectorDS.run([
        [xmin, ymin],
        [xmax, ymax],
    ...
    function handleSelection(): void {
      if (selectionMode !== EnumSelectionMode.none) handleEscape()
    ...
    function handleSpace(): void {
      ...
      const selectorDS = (getSelectorDataSource() ?? new SelectorDataSource()) as SelectorDataSource
      selectorDS.clear()
      const boundaryDpr = DeviceUtils.getDPR()
      const boundaryWithDpr: Point2D[] = boundary.map(
        (arr) => arr.map((item) => item * boundaryDpr) as Point2D,
      )
      selectorDS.run(boundaryWithDpr)
      models.forEach((model) => {
    ...
    function handleEscape(): void {
      switch (selectionMode) {
        case EnumSelectionMode.rect: {
          ...
          models.forEach((model) => model.highlightSelectionPointsFilter.setSelectionBox([]))
          break
        }
    ...
    function getPointerPosition(e: MouseEvent, el: HTMLElement): { x: number; y: number } {
    ...
    function handleRightClick(callData: AnyObj): void {
      ...
      if (!picker.getActors().length) return

      const pickedCellId: number = picker.getCellId()
      if (selectToothIdx) selectToothIdx.delete()
      const containerHeight = DeviceUtils.getContainerHeight()
      const rightDpr = DeviceUtils.getDPR()
      const winPos: [number, number] = [pos.x / rightDpr + 3, 3 + containerHeight - pos.y / rightDpr]
      selectToothIdx = new SelectToothIdx(
        ...
        (toothIdx) => {
          const idx =
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ((picker.getActors()[0] as any)?.getMapper()?.getViewSpecificProperties() as any)?.["name"] == "l" ? 0 : 1
          const hlCells = getHlCells()
          if (hlCells?.length) {
            for (let i = 0; i < hlCells.length; i++) {
              models[idx].vertexSculptureFilter.setCellInfo(hlCells[i], toothIdx)
            }
    ...
    function getScreenMapModelPos(point: { x: number; y: number }): Point3D {
      ...
      if (!picker.getActors().length) return [0, 0, 0]
    ...
    function getSinleNowFile(index: number): string {
      return getSingleNowFile(index)
    }

    function getSingleNowFile(index: number): string {
      const writer = vtkXMLPolyDataWriter.newInstance()
      writer.setFormat(vtkXMLWriter.FormatTypes.ASCII)
      return writer.write(models[index].vertexSculptureFilter.getCachedPolyData())
    }

    function triggerFinalFile(buffers: string[]): void {
    ...
    function triggerFile(buffer: string, idx: number): void {
      const textEncoder = new TextEncoder()
    ...
    function initSpline(index: number): void {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const widgetRepresentation = widgetManager.addWidget(spline) as any
    ...
    function addSpline(points: Point3D[], index: number, _normal: Point3D): void {
      if (!splineWidgets[index]) initSpline(index)
    ...
    function setSplineWidgetVisible(value: boolean, index: number): void {
      ...
      splineWidgets[index]!.spline.setVisibility(value)
      renderWindow.render()
    }
    ...
    function addTextTag(center: Point3D, name: string): AnyObj {
      ...
      psMapper.setCallback((coordsList: number[][]) => {
        if (textCtx && container) {
          const textDpr = DeviceUtils.getDPR()
          const [x1, y1] = coordsList[0],
            [x2, y2] = coordsList[Math.floor(coordsList.length / 2)],
            x = (x1 + x2) / textDpr / 2,
            y = container.clientHeight - (y1 + y2) / textDpr / 2
    ...
    function renderToothOrder(
      points: ToothOrderPoint[],
      viewUp: Point3D,
      modelIdx: number,
    ): void {
      ...
      if (JSON.stringify(toothOrders[key]) === "{}") {
        points.forEach((item) => {
          toothOrders[key][item.name] = { face: null, num: null }
        })
      }
      let maxPosVal = 0,
        maxPosIdx = 0
    ...
    function addToothOrder(
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ;(cylinderSource as any).setCenter(...center)
      ...
      renderer.addActor(actor)
      toothOrders[key][name] = { cylinderSource, actor, mapper, textActor, psMapper }
    }

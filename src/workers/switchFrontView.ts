...
function createVector3(): Point3D {
    return [0, 0, 0]
}
...
function switchFrontViewWorker({ obbData, boundary, meshData }: WorkerInput): [Point3D, Point3D, Point3D] {
    let [norm, vaxis, center] = checkLeft(d1Tod3(obbData) as Point3D[])
    ...
    if (temp > 50) {
        vaxis = vtkMath.multiplyScalar(vaxis, -1) as Point3D
    }
    ...
    const pointAtFrontFace = findPointAtFrontFace(meshData, [...norm, -(vtkMath.dot(norm, uCenter))] as [number, number, number, number], [uCenter, center1])
    ...
    if (temp2 > 90) {
        z = vtkMath.multiplyScalar(z, -1) as Point3D
    }

    vtkMath.add(center, vtkMath.multiplyScalar(z, 150.5) as Point3D, pos)
    return [center, vaxis, pos]
}

function checkLeft(points: Point3D[]): [Point3D, Point3D, Point3D] {
    ...
    if (j === -1) {
        return [createVector3(), createVector3(), createVector3()]
    }
    ...
    c = vtkMath.multiplyScalar(c, 0.5) as Point3D

    return [h, v, c]
}

function distanceBetweenPointToPlane(point: Point3D, points: Point3D[]): number {
    ...
    const [a, b, c] = cp
    const d = -vtkMath.dot(cp, p3)
    return distanceBetweenPointToPlaneV2(point, [a, b, c, d])
}

function isPointInPlane(points: Point3D[], point: Point3D): boolean {
    return Math.abs(distanceBetweenPointToPlane(point, points)) < 0.00001
}

function normalOfPlane(points: Point3D[]): Point3D {
    const [p1, p2, p3] = points
    const v1 = createVector3()
    vtkMath.subtract(p3, p1, v1)
    const v2 = createVector3()
...
function constructArr1(points: Point3D[]): [Point3D[], Point3D[]] {
    const p1 = points[0]
    const arr: [number, number][] = [
        [1, getDistance(p1, points[1])],
        [2, getDistance(p1, points[2])],
    ]
    ...
    const idxArr = [0, ...arr.map((a) => a[0])]
    const points1 = points.filter((_, idx) => !idxArr.includes(idx))

    return [[p1, ...arr.map((a) => points[a[0]])], points1]
}
...
function massCenterOfCube(points: Point3D[]): Point3D {
...
function findPointAtFrontFace(data: number[], planeDef: [number, number, number, number], centerLine: [Point3D, Point3D]): Point3D {
    const points = d1Tod3(data) as Point3D[]
    const arr: Point3D[] = []
    const maxItemCount = 5
    for (let i = 0; i < points.length; i++) {
        if (arr.length >= maxItemCount) break
        if (
            distanceBetweenPointToPlaneV2(points[i], planeDef) < 1 &&
            distanceBetweenPointToLine(points[i], centerLine) < 1
        ) {
            arr.push(points[i])
        }
    }
    if (arr.length < 3) {
        return points[0].map((n) => -n) as Point3D
    } else {
        const arr1: [number, number, number][] = [
...
function distanceBetweenPointToLine(x0: Point3D, pointsOfLine: [Point3D, Point3D]): number {

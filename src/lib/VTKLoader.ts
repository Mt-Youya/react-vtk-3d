...
/** 与 parseVTP.ts 完全一致的 26 色 LABEL_LUT */
	...
	parse( data ) {
		...
		function parseXML( stringFile ) {
			...
			if ( json.PolyData ) {
				...
				// Apply per-cell scalar colors using LABEL_LUT (expand to per-vertex after toNonIndexed)
				if ( cellScalars.length > 0 ) {
					...
					for ( let i = 0; i < triCount; i ++ ) {
						...
						const [ r, g, b ] = labelToRGB( scalar );
						const rf = r / 255, gf = g / 255, bf = b / 255;

						for ( let v = 0; v < 3; v ++ ) {

							colorAttr[ ( i * 3 + v ) * 3 ] = rf;
							colorAttr[ ( i * 3 + v ) * 3 + 1 ] = gf;
							colorAttr[ ( i * 3 + v ) * 3 + 2 ] = bf;
					...
					// 将 cellScalars 作为自定义属性存储，供外部提取
					const labelsAttr = new Float32Array( triCount );
					for ( let i = 0; i < triCount; i ++ ) {
						labelsAttr[ i ] = cellScalars[ i ] !== undefined ? cellScalars[ i ] : 0;
					}
					geometry.userData.cellLabels = labelsAttr;
...
export interface VTPParseResult {
...
/**
 * 用 VTKLoader 解析 VTP ArrayBuffer。
 *
 * - 使用 VTKLoader 的 XML 解析逻辑（支持 appended + compressed 格式）
 * - 颜色映射使用 LABEL_LUT 26 色（牙科专用配色）
 * - cellLabels 从 geometry.userData.cellLabels 提取
 * - positions/colors 从 geometry.attributes 提取（已 toNonIndexed 展开）
 */
export function parseVTPWithVTKLoader(buffer: ArrayBuffer): VTPParseResult {
	const loader = new VTKLoader();
	// 直接调用内部 parse 方法（跳过 URL 加载，直接传 ArrayBuffer）
	let geometry = loader.parse(buffer) as BufferGeometry;

	// 确保 geometry 是非索引的（toNonIndexed 展开），保证 positions 是平铺数组
	// 如果已经是 non-indexed（cellScalars 分支已调用），index 为 null，此处是 no-op
	if (geometry.index !== null) {
		const cellLabels = geometry.userData.cellLabels as Float32Array | undefined;
		geometry = geometry.toNonIndexed();
		if (cellLabels) geometry.userData.cellLabels = cellLabels;
	}
	...
	const cellLabels = (geometry.userData.cellLabels as Float32Array | undefined) ?? null;

	return { positions, normals, colors, cellLabels };
}

...
/**
 * 将 ArrayBuffer（VTP/VTK 格式）解析为顶点数据
 * 供 ToothObject.ts 使用
 */
export async function parseVTP(buffer: ArrayBuffer): Promise<{
  positions: Float32Array
  normals: Float32Array | null
  colors: Float32Array | null
}> {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const loader = new (VTKLoader as any)()
  // VTKLoader.parse 是同步方法，返回 BufferGeometry
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const geo: any = loader.parse(buffer)
